...
usage() {
  cat <<'EOF'
Usage: insta360_video_speed_gpx.sh [--dry-run] VIDEO.mp4 STRAVA.gpx [OUTPUT.fit]

Options:
  --dry-run        Auto-sync and write original GPS-derived motion without
                   replacing it with optical-flow speeds.
...
Environment variables:
  ...
  FLOW_WORKERS     Parallel optical-flow workers (default: 4)
...
python_script="$script_dir/video_speed_gpx.py"
fit_converter="$script_dir/gpx_to_fit.py"
...
[[ -f "$input_gpx" ]] || { echo "Error: Strava GPX not found: $input_gpx" >&2; exit 2; }
[[ -f "$python_script" ]] || { echo "Error: processor not found: $python_script" >&2; exit 2; }
[[ -f "$fit_converter" ]] || { echo "Error: FIT converter not found: $fit_converter" >&2; exit 2; }
...
intermediate_gpx=$(mktemp -t insta360_video_speed)
cleanup() {
  rm -f "$intermediate_gpx"
}
trap cleanup EXIT
...
python3 "$fit_converter" "$intermediate_gpx" "$output_fit"
